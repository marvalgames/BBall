...
void CDigiStatic::OnPaint() 
{
	...
	CPen *pOldPen = MemDCDraw.SelectObject(&OffPen);
	...
	for (int iChar = 0; iChar < m_CharArray.GetSize(); iChar++)
	{
		...
		m_CharArray[iChar].Draw(&MemDCDraw, CharRect, &OffPen, &OnPen, &hBrushOff, &hBrushOn, region);
	...
	MemDCDraw.SelectObject(pOldPen);
	MemDCDraw.SelectObject(pOldBrush);
	OffPen.DeleteObject();
	OnPen.DeleteObject();
...
BOOL CDigiStatic::OnEraseBkgnd(CDC* /*pDC*/) 
{
	...
	// Background is painted in CMemDCDraw::CMemDCDraw(); with FillSolidRect();
	return FALSE;
}
